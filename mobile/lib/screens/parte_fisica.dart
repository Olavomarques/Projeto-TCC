...
class _ParteFisicaState extends State<ParteFisica> {
  ...
  Future<void> _carregarTreinos() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final treinosDoUsuario = todosOsTreinos.where((treino) {
          // Tenta pegar o id_user do treino de diferentes formas
          final treinoUserId = treino['id_user']?.toString() ?? 
                              treino['userId']?.toString() ?? 
                              treino['user_id']?.toString();
          
          print('Treino: ${treino['nome']} - UserID do treino: $treinoUserId - Meu ID: $userId');
          
          // Compara com o ID do usuário logado
  ...
  Future<void> _salvarNovoTreino() async {
    ...
    try {
      ...
      // ✅ CORRETO: Usa id_user e converte para número
      final Map<String, dynamic> dadosTreino = {
        'nome': _nomeTreinoController.text,
        'id_user': int.parse(userId!), // ✅ CONVERTE PARA NÚMERO
      };
  ...
  void _irParaExercicios() {
    Navigator.push(
      context,
      MaterialPageRoute(builder: (context) => const Exercicios()), // ✅ CORRIGIDO: "Exercicios" com E maiúsculo
    );
  }
                  ...
                  SizedBox(
                    ...
                    child: OutlinedButton(
                      ...
                      child: const Row(
                        mainAxisAlignment: MainAxisAlignment.center,
                        children: [
                          Icon(Icons.sports_gymnastics, color: Color(0xFF5BA0E0)),
                          SizedBox(width: 8),
                          Text(
                            'EXERCÍCIOS',
                            style: TextStyle(
