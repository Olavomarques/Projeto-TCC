...
class _HomePageState extends State<HomePage> {
  ...
  int? _diarioId;
  List<Map<String, dynamic>> _historicoDiarios = []; // ✅ HISTÓRICO DE DIÁRIOS
  ...
  void initState() {
    ...
    _carregarHistoricoDiarios(); // ✅ CARREGA HISTÓRICO
  }
  ...
  Future<void> _carregarDiarioDoDia() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        // ✅ DEBUG: Mostra a estrutura dos diários
        ...
        // ✅ CORREÇÃO: Verifica se diarios é uma lista e procura o diário do usuário
        if (diarios is List) {
          Map<String, dynamic>? diarioDoUsuario;
          
          try {
            // ✅ FORMA CORRETA: Usando um loop for para encontrar o diário
            for (var diario in diarios.cast<Map<String, dynamic>>()) {
              try {
                final diaDiario = DateTime.parse(diario['dia']).toLocal();
                final diaDiarioFormatado = DateTime(diaDiario.year, diaDiario.month, diaDiario.day);
                ...
                continue; // Continua para o próximo diário se houver erro
              }
            }
          } catch (e) {
  ...
  Future<void> _carregarHistoricoDiarios() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (diarios is List) {
          // ✅ FILTRA DIÁRIOS DO USUÁRIO E ORDENA POR DATA (mais recente primeiro)
          final diariosDoUsuario = diarios.cast<Map<String, dynamic>>()
    ...
    try {
      ...
      if (response.statusCode == 201 && mounted) {
        ...
        setState(() {
          ...
          // ✅ CORREÇÃO SEGURA: Verifica se o campo existe
        ...
        // ✅ ATUALIZA O HISTÓRICO APÓS CRIAR NOVO DIÁRIO
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('Erro ao criar diário: ${response.statusCode} - ${response.body}'),
              backgroundColor: Colors.red,
            ),
          );
        }
  ...
  Future<void> _atualizarCoposAgua(int novosCopos) async {
    ...
    try {
      ...
      if (response.statusCode == 200 && mounted) {
        ...
        print('❌ Erro ao atualizar copos: ${response.statusCode} - ${response.body}');
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
  ...
  Future<void> _atualizarExercicios(int novosExercicios) async {
    ...
    try {
      ...
      if (response.statusCode == 200 && mounted) {
        ...
        print('❌ Erro ao atualizar exercícios: ${response.statusCode} - ${response.body}');
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('Erro ao atualizar exercícios: ${response.statusCode}'),
              backgroundColor: Colors.red,
            ),
          );
        }
  ...
  // ✅ DIMINUIR EXERCÍCIOS (NOVA FUNCIONALIDADE)
      ...
      body: SafeArea(
        child: Column(
          children: [
            ...
            // ✅ EXERCÍCIOS FÍSICOS COM BOTÕES + E -
            ...
            // ✅ CALORIAS GASTAS (APENAS LEITURA)
            _buildItemDiario(
          ...
          Row(
            children: [
              ...
              if (onAdd != null) ...[
                const SizedBox(width: 10),
                IconButton(
                  icon: const Icon(Icons.add_circle, color: Color(0xFF5BA0E0), size: 28),
                  onPressed: onAdd,
  ...
  // ✅ NOVO WIDGET PARA EXERCÍCIOS COM CONTROLE COMPLETO
 ...
 // ✅ MÉTODO PARA MOSTRAR HISTÓRICO DE DIÁRIOS (ATUALIZADO)
  ...
  showDialog(
    ...
    builder: (context) => AlertDialog(
      ...
      content: SizedBox(
        ...
        child: _historicoDiarios.isEmpty
...
// ✅ MÉTODO PARA MOSTRAR DETALHES DE UM DIÁRIO ESPECÍFICO (SEM INTl)
void _showDetalhesDiario(BuildContext context, Map<String, dynamic> diario, bool isHoje) {
  // Formata a data manualmente sem usar intl
  final data = DateTime.parse(diario['dia']);
  final dataFormatada = '${data.day.toString().padLeft(2, '0')}/${data.month.toString().padLeft(2, '0')}/${data.year}';
  ...
  if (diario.containsKey('lastupdate')) {
    ...
    ultimaAtualizacao = '${updateTime.hour.toString().padLeft(2, '0')}:${updateTime.minute.toString().padLeft(2, '0')}';
  }
  
  showDialog(
    context: context,
    builder: (context) => AlertDialog(
      title: Text(isHoje ? 'Diário de Hoje' : 'Diário do dia $dataFormatada'),
      content: Column(
