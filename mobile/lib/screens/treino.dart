...
class _TelaTreinoState extends State<TelaTreino> {
  ...
  Future<void> _carregarExerciciosDoTreino() async {
    try {
      ...
      if (responseTreinosLink.statusCode == 200 &&
        ...
        final treinosLinksFiltrados = todosTreinosLinks.where((link) {
  ...
  void _editarExercicio(dynamic exercicio) {
    showDialog(
      context: context,
      builder: (context) => DialogEditarExercicio(
        exercicio: exercicio,
        onSalvar: (novasSeries, novasRepeticoes, novoPeso) {
          _atualizarExercicio(exercicio, novasSeries, novasRepeticoes, novoPeso);
        },
      ),
    );
  }
  ...
  void _excluirExercicio(dynamic exercicio) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text('Tem certeza que deseja excluir este exercício do treino?'),
        actions: [
          TextButton(
  ...
  Future<void> _atualizarExercicio(dynamic exercicio, int series, int repeticoes, double? peso) async {
    try {
      final response = await http.put(
        Uri.parse('https://backend-tcc-iota.vercel.app/exerselec/${exercicio['id_ExerSelec']}'),
      ...
      if (response.statusCode == 200) {
        _carregarExerciciosDoTreino(); // Recarregar a lista
      } else {
  ...
  Future<void> _confirmarExclusao(dynamic exercicio) async {
    try {
      final response = await http.delete(
        Uri.parse('https://backend-tcc-iota.vercel.app/treinolink/${exercicio['id_link']}'),
      );
  ...
  void _adicionarExercicio() {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => ExerciciosSelecao(
          ...
          children: [
            Expanded(
              child: ElevatedButton(
                onPressed: () {
                  ...
                  _editarExercicio(exercicio); // Abrir modal de edição
                },
            ...
            Expanded(
              child: ElevatedButton(
                onPressed: () {
                  Navigator.pop(context); // Fechar modal atual
                  _excluirExercicio(exercicio); // Abrir confirmação de exclusão
                },
                style: ElevatedButton.styleFrom(
      ...
      body: SafeArea(
        child: Column(
          children: [
            ...
            Expanded(
              child: _buildConteudoExercicios(),
            ),
          ],
        ),
      ),
...
class _DialogEditarExercicioState extends State<DialogEditarExercicio> {
  ...
  void initState() {
    super.initState();
    seriesController = TextEditingController(text: widget.exercicio['series']?.toString() ?? '3');
    repeticoesController = TextEditingController(text: widget.exercicio['repeticoes']?.toString() ?? '10');
